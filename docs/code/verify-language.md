...
The lookarounds spare what is not a number: a regex quantifier `{1,3}`, a CSS `rgba(16,21,28,.06)`, an awk `substr($0,3,36)`. 🔴 Exactly three digits after the comma are NOT judged, and the limit is structural rather than chosen: `6,766` and `0,078` have the same shape, so nothing here tells a thousands separator from a three-decimal number. That convention was never settled either.
