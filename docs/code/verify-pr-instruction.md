...
⚠️ **Known and counted**: an order carried by its expected *result* — "une PR par repo", "promeus" — names no opening verb, and one order can command several openings. The pattern matched **40 of 1281**
real messages; the forms it misses are listed in the research. As an instrument this only skews the measure slightly pessimistic. It would matter if this ever became a gate.
