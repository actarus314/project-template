...
Nothing else watches the TURN: the thirteen other checks watch
...
the conversation. The third signal exists because a measurement can be rendered and lost: timings
...
What a turn CLAIMS, against what it actually ran: a defect stated with nothing edited, a
total quoted that appears in no output, measurements rendered and never written.
