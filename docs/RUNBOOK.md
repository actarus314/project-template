...
| 2 | Claude | **Seal `CHANGELOG.md`**: `Unreleased` becomes `## [X.Y.Z] - <date>`, and `.claude-plugin/plugin.json` gets the same number. Through a pull request — **opened on the maintainer's instruction**. |
| 3 | Claude | **THEN** `git tag vX.Y.Z && git push origin vX.Y.Z` → the **Release** is created, and the **ghcr image** pushed *(if `--artefact`)*. ⚠️ **With `--artefact`, the Release is the `release` job of `docker-publish.yml`** *(`needs: build-push` — no Release if the image was not published)*; **without** this capability, it is `release.yml`. **Only one of the two exists**, never both. **Neither one writes the note**: a tag push has nothing to write, so both publish **the two links alone** and say so — **the written half is added by hand afterwards** *(what a Release owes: `METHODE.md`)*. ⚠️ **A repo with NEITHER workflow** *(this one)* **creates the Release by hand, note included** — the text is written first, then handed to the script on **stdin**, in **two chained commands**, never a `<(…)`, which discards the script's exit code and would publish an **empty** body as a success: `./release-notes.sh vX.Y.Z < note.md > notes.md && gh release create vX.Y.Z --title vX.Y.Z --notes-file notes.md`. |
