...
| 2b | Claude | **Version `CLAUDE.md`, reduced to the `@AGENTS.md` import** — take it out of `.gitignore`, strip it of anything personal, `git add` it. Gitignored, it reaches nobody who clones, and **it is the only thing that loads `AGENTS.md`** *(measured: an agent in a clone did not have that file in its context at all)*. **What it must not carry, and why the rule is worded that way: the standard §6.** ⚠️ **Read what is in it before adding it** — machine paths, private repository names, personal preferences: all of that has to leave first, and the history keeps whatever is pushed. |
