...
🔴 **The COMMIT gate is another matter, and there it does lie — commit from the main tree until this is closed.** Git hands a hook `GIT_INDEX_FILE` **relative** in an ordinary repository and **absolute** in a worktree; absolute, it survives `git -C`, so every check reading the neighbour reads **this** repository's index instead. Measured: `verify-workspace.sh` then reports a secret-named file and stray open work that exist here and not there — and the same variable set by hand, from the main tree, reproduces it exactly. **The guard on `workspace/` holds by the accident that this path is usually relative**, which is worth knowing before trusting any verdict it gives from a worktree.
