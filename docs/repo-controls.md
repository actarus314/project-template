...
| `checks/verify-language.sh` | **accented** French left in published content, **and the French decimal comma** — the language, which its neighbour above never looked at | repo/ *(English stops where publication stops)* | every commit | ✅ | needs watching — both signals are binary, but the decimal one's anchors are written by hand, to spare a regex quantifier, a CSS colour and an awk argument list. Unaccented French PROSE stays uncovered **deliberately**: swept for, nothing had been left behind, and a detector would cost the checks their one invariant. A comma followed by exactly three digits is not judged: a thousands separator and a three-decimal number share that shape | 0.09 s | ✅ |
