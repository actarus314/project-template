...
| `checks/verify-workspace.sh` | two tracking systems competing, a secret tracked by git, **and open work on either side of the tracking doc** — a closed item inside it, an action outside it in a file pledging to hold none or in an archive being written | both | every commit | ✅ | **needs watching** — the list of rival tools cannot be complete, and it names what it looked for. Both hygiene halves match a FORM, never a state: an unmarked closed item, or a leftover that never declares itself, stays invisible | 0,11 s | ✅ |
...
| `checks/verify-do-not-break.sh` | the wirings whose breakage makes no sound — **including a versioned `CLAUDE.md` losing its import, and the one address a generated project has for the documents it never receives** | both | every commit | ✅ | settled | 0,06 s | ✅ |
