...
| `checks/verify-checksums.sh` | an `.html` that stopped saying what its `.md` says, here **or in the neighbouring workspace** | both | every commit | ✅ | **needs watching** — a matching checksum proves the file was touched, never that it says the same thing | 0.08 s | ✅ |
