...
| `checks/verify-orphan-branch.sh` | a local branch that **never had an upstream**, and what its content is worth — reachable from a remote ref *(bin it)* against found nowhere *(it is the only copy)* | local | every commit | ✅ | **needs watching** — the branch CHECKED OUT is exempt, or the check would speak at every commit of ordinary work, which is how a guard gets disarmed. Its second instrument is patch-id, blind to a rebase that resolved a conflict — the error then falls on the safe side, *only copy*. The age of a dormant branch is **published and never judged**: no measurement here says when it becomes a problem | 0,04 s | ⚠️ advisory — a branch not pushed yet is ordinary work, and only the maintainer tells which is which |
