...
| `checks/verify-housekeeping.sh` | the development admin behind the work: commits piling up with nothing written down | local | end of turn | ✅ | **needs watching** — the threshold is MEASURED *(4 commits: speaks once every 2,1 days, against a hand-asked pass every 2,3)*, and it routes to the `housekeeping` skill rather than deciding anything itself | instant | n/a — asks for the pass before the turn ends |
