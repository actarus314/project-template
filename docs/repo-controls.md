...
| `checks/verify-commit-form.sh` | a commit subject past 72 characters, uncapitalised, ending on a full stop, opening on an article, or with its body glued to line 2 | both | every commit | ✅ | **needs watching** — 🔴 it does **not** govern `main`: merging is `--squash`, so the subject landing there is the pull-request title *(measured: all 40 of the last, 25 of them past 72)*, refused by `open-pr.sh` instead. Bots and git's own subjects are exempt, or every Renovate pull request would go red | 0,05 s | ✅ |
