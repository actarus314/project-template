...
| `checks/verify-pr-instruction.sh` | a pull request opened with no instruction from the maintainer | local | on every prompt, and before every command that opens one | ✅ | **an instrument, not a guard** — it records and returns, never refuses. Its threshold is written before its data: under **5 %** without an instruction over **20** openings, the token does not get built | instant | n/a — a hook, and it never refuses: measuring is its job |
...
| `checks/verify-rules-read.sh` | writing before the rule documents have been READ — they are injected as context, so nothing traces whether they were ever obeyed | both | `SessionStart` arms it; every `Write`/`Edit` landing in the repo checks it | ✅ | **needs watching** — the threshold is not a number but a definition: a read carrying `offset`/`limit` does **not** count, since a passage is not the document. Documents are DETECTED, so a generated project holding none stands down. Re-armed at every `SessionStart`, **compaction included** — that is the case it exists for | instant | n/a — a hook, it refuses the write |
