...
| **Reported content** (moderation) | **UI-only — NO API** (verified: no moderation endpoint exists). Exact path, scope (org only) and item count: **`github-repo-config.md` §5, point 6**. ⚠️ The box **is not checked** on "All users". |
...
- **ORG repo — SYSTEMATIC, never an exception**: the "Reported content" moderation setting is **UI-only** (no REST/GraphQL API) and is only applied by default to repos **created public** — so **never to ours**, born private. Without it, community health caps out. **Exact path + value: RUNBOOK §5 · `github-repo-config.md` §5.6.**
