...
**Cost: 2,1 s — a floor, never a ceiling**, `--commit` being the mode that runs the external tools: a cold cache or no network refuses the commit. *(Why the gate lives in `repo/`, and what it buys: [`docs/code/init-project.md`](code/init-project.md).)*
