...
- 🔴 **What decides is DECOMPOSABILITY into units that do not touch each other** — never "multi-agent or not". Where sub-tasks share files or interfaces, success falls monotonically as agents are added: **68,6 %** at two, **46,5 %** at three, **30,0 %** at four *(CooperBench, ICLR workshop 2026 — 650 real software tasks, agents sharing one repository and having to merge)*.
  ⚠️ **That benchmark measures PEER-TO-PEER coordination on one codebase — the opposite of an orchestrator fanning out**, and the distinction is what makes it compatible with the favourable case: genuinely disjoint, read-only work *(several angles of one search)*, which Anthropic names as its own — alongside CODE as its unfavourable one.
- **Prefer workflows** *(deterministic orchestration: parallel fan-out, pipeline, adversarial verification)* **wherever that condition holds**: a task that breaks down into parallel tasks or verifiable steps benefits from being a workflow rather than one long sequential pass.
