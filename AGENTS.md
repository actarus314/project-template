...
- 🔴 **Open a pull request only on an instruction to open one, and NAME that instruction when doing it** —
  ...
  instruction and never has; opening does. *(Measured over this repository's transcripts: **49 %** of
  pull requests were opened with no instruction traceable to the maintainer.)*
