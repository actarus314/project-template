...
The `pre-commit` hook **reruns it on its own, at every commit, and now BLOCKS on a gap**: it replays `./check.sh --commit` (~1s) each time, and the full lot every 6h *(gitleaks on staged files was already blocking)*. To adjust the 6h delay: `CHECK_MAX_AGE_HOURS`. Escape hatch: `git commit --no-verify`, a decision, not an accident.
