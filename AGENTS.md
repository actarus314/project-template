...
- **A relative link and its anchor** resolve, and a pointer crediting a document with a quoted formula is checked against it. **An `.html` view** names an `.md` that exists, and follows it whenever it changes — **wherever the pair lives**, the neighbouring workspace included. **A path a memory names still resolves**, a placeholder and a rank excepted.
