...
It sits at the **root**, never under `templates/` — and **not** because `init-project.sh` only copies from there. It also copies `check.sh`, `open-pr.sh` and **the whole of `checks/`** *(`cp "$TPL/checks/"verify-*.sh`, since every check detects its own perimeter and none has to be picked)*. What protects the skill is that the copies are **named**: two files, plus one glob confined to `checks/verify-*.sh`. `skills/` is reached by none of them.
...
🔴 **`./check.sh --house` is the CI's door** — the house checks and nothing else, since the CI pins and runs the external tools itself. **One line, in every gating workflow**: this repo's `ci.yml` and the three `templates/workflows/ci-*.yml`. A check added to `checks/` is therefore gated **everywhere, with no workflow to edit** — and `verify-checks-wiring.sh` fails the build if that line ever leaves one of them. *(Naming the checks one step at a time is what let a check ship, run nowhere and gate nothing.)* What passes locally passes the CI — but **the CI remains the authority** *(it alone verifies the SHA256 of Linux assets and runs under real conditions)*.
