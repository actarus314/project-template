...
# This check travels like every other one, and its subject is the GENERATOR. A project generated by
# this template holds no generator, so there is nothing here to generate FROM — that is not a defect
# and must not read as one. Said out loud rather than by an early `exit 0`: without the generator,
# the two reads below come back empty, and an empty read is what the next two lines FAIL on.
...
for label, gendir in gens:
    ...
    for g in sorted(p for p in gen.rglob("*") if p.is_file() and not any(s in p.parts for s in SKIP)):
        try:
            ...
            continue
        # A file that TESTS a path knows it may be missing, and this honours that claim. There are
        # three ways of making it, and all three count now that the checks themselves travel:
        #   · a shell test, negated or not — `[ -f x ]`, `[ ! -f x ]`
        #   · a Python existence test — `pathlib.Path("x").exists()`
        #   · a literal bound to a NAME that is tested elsewhere in the same file. The test and the
        #     literal sharing a line was an accident of how the first guarded files were written;
        #     requiring it reported three checks that guard their targets perfectly well.
