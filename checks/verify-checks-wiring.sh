...
if RUNNER.exists() and HOOK.exists():
    ...
    r, h = in_code(RUNNER), in_code(HOOK)
    if r and not h:
    ...
    # Gone from BOTH is the shape that passes in silence: no half contradicts the other, and the
    # line below would still publish the coupling as read.
    if not r and not h:
        bad.append(f"neither {RUNNER} nor {HOOK} names {VAR} — nothing tells a commit made here "
                   f"from one made next door, and a workspace commit wakes both generating checks")
